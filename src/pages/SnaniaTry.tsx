...
import { createSimulation, listSimulations, type SimulationSummary } from '../lib/api'
...
import { toWorkingCanvas, warpFace, whitenTeeth } from '../lib/simulate'
...
export default function SnaniaTry() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const points = await detectFaceLandmarks(canvas)
      if (!points) {
        setStatus('no-face')
        return
      }
  ...
  function generate() {
    ...
    // Let the "generating" state paint before the (synchronous, CPU-bound) warp runs.
    requestAnimationFrame(() => {
      ...
      if (selectedProcedures.some((p) => p.whitening)) {
        whitenTeeth(colorCtx, landmarks, width, height)
      }

      const warpProcedures = selectedProcedures.filter((p) => p.warp && p.warp.length > 0)
      let finalCanvas = colorCanvas
      if (warpProcedures.length > 0) {
        const warped = document.createElement('canvas')
        warped.width = width
        warped.height = height
        warpFace(colorCanvas, warped.getContext('2d')!, landmarks, warpProcedures, width, height)
        finalCanvas = warped
      }

      setBeforeUrl(beforeCanvas.toDataURL('image/jpeg', 0.88))
      setAfterUrl(finalCanvas.toDataURL('image/jpeg', 0.88))
      setReveal(50)
      setStatus('result')
    })
  }
    ...
    <section className="relative bg-soft-gray pt-28 pb-24 lg:pt-36">
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        ...
        <div className="mt-6 flex items-start gap-3 rounded-2xl border border-amber-300/50 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <p>
            <strong>Prototype demo, not a medical device.</strong> This is an
            AI-estimated preview for demonstration only &mdash; not a
            clinical prediction. Please don&rsquo;t upload real patient
            photos: anything you choose to save here goes into a shared demo
            database visible in the gallery below.
          </p>
        </div>
